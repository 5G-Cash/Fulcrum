...
struct uint512 {
    ...
    uint8_t* end() { return data.data()+64; }
    const uint8_t* end() const { return data.data()+64; }
    bitcoin::uint256 trim256() const {
        bitcoin::uint256 ret{bitcoin::uint256::Uninitialized};
        std::memcpy(ret.begin(), data.data(), 32);
        return ret;
    }
};
#include "sph_blake.h"
...
#define START_OF_LAST_16_NIBBLES_OF_HASH 48
    const std::string hex = PrevBlockHash.ToString();
    const char c = hex[START_OF_LAST_16_NIBBLES_OF_HASH + index];
