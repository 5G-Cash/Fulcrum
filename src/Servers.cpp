...
namespace {
    ...
    QString transformDefaultDonationAddressToBTCOrBCHOrLTCOrVGC(const Options &options, bool isNonBCH, bool isLTC, bool isVGC)
    {
        ...
        if (!ret.isEmpty()) {
            try {
                ...
                if (addr.isValid()) {
                    ...
                    else if (isVGC) ret = addr.to5GCashString();
            ...
            if (s[i] == sigil) {
                ...
                for (qsizeT len = smallestSubst, sz = s.size(); len <= largestSubst && i + 1 + len <= sz; ++len) {
                    ...
                    if (const auto it = std::as_const(substs).find(substr); it != substs.end()) {
                        // match! replace the sigil plus substring  of length `len` with the substitution string
...
void Server::rpc_server_banner(Client *c, const RPC::BatchId batchId, const RPC::Message &m)
{
    ...
    if (bannerFile.isEmpty() || !QFile::exists(bannerFile) || !QFileInfo(bannerFile).isReadable()) {
        ...
        generic_do_async(c, batchId, m.id,
                        [bannerFile,
                         donationAddress = transformDefaultDonationAddressToBTCOrBCHOrLTCOrVGC(*options, isNonBCH(), isLTC(), isVGC()),
...
{
    emit c->sendResult(batchId, m.id, transformDefaultDonationAddressToBTCOrBCHOrLTCOrVGC(*options, isNonBCH(), isLTC(), isVGC()));
}
/* static */
